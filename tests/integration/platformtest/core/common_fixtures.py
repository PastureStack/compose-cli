...
def _patch_gdapi_rest_object_compat():
    # cattle>=0.5.4 imports on Python 3, but gdapi RestObject no longer
    # exposes the list-like helpers that these legacy tests relied on.
    try:
        import gdapi
    except ImportError:
        return

    cls = gdapi.RestObject
...
_patch_gdapi_rest_object_compat()
...
def make_new_context(admin_user_client, request):
...
def new_context(admin_user_client, request):
    return make_new_context(admin_user_client, request)
...
class Context(object):
    ...
    def _get_nsp(self):
        ...
        networks = [
            x for x in self.client.list_network(kind='hostOnlyNetwork')
            if x.kind == 'hostOnlyNetwork' and x.accountId == self.project.id
        ]
        assert len(networks) == 1
        nsps = _get_super_client(None).reload(
            networks[0]).networkServiceProviders()
        assert len(nsps) == 1
        return nsps[0]
    ...
    def super_create_container_no_success(self, *args, **kw):
        ...
        return self._create_container(_get_super_client(None), *args, **kw)
...
def register_simulated_host(client_or_context, return_agent=False):
    ...
    def do_ping():
        ping = one(_get_super_client(None).list_task, name='agent.ping')
        ping.execute()

    def check():
        hosts = _get_super_client(None).list_host(agentId=agents[0].id)
        if len(hosts) > 0:
            return hosts[0]
        do_ping()
...
def get_plain_id(admin_client, obj=None):
    if obj is None:
        obj = admin_client
        admin_client = _get_super_client(None)
