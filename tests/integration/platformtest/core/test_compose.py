...
def client(admin_user_client, request):
    ...
    return make_new_context(admin_user_client, request).client
...
def _normalize_data_volume(volume):
    # Rancher 1.6 may return Docker's implicit read-write mode explicitly.
    if volume.endswith(':rw'):
        return volume[:-3]
    return volume


def _port_rule_weight(rule):
    # The API omits the default LB weight; compose semantics still default to
    # one.
    return getattr(rule, 'weight', 1)
...
def test_args(client, compose):
    ...
    for project in (project_with_build, project_with_build_v2,
        ...
        normalized_volumes = [
            _normalize_data_volume(v) for v in launch_config.dataVolumes
        ]
        assert normalized_volumes == ['/tmp/foo', '/tmp/x:/tmp/y']
...
def test_circular_sidekick(client, compose):
    ...
    secondary = [
        x for x in service.secondaryLaunchConfigs if x.name == 'secondary'
    ]
...
def test_extends_1556(client, compose):
    ...
    assert web.launchConfig.imageUuid == 'docker:ubuntu:26.04'
    assert db.launchConfig.imageUuid == 'docker:ubuntu:26.04'
...
def test_lb_regions_upgrade(client, compose):
    ...
    assert _port_rule_weight(lb.lbConfig.portRules[1]) == 1
    ...
    assert _port_rule_weight(lb.lbConfig.portRules[0]) == 1
    assert lb.lbConfig.portRules[0].serviceId == web.id
