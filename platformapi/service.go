...
func (r *PlatformService) Name() string {
	return r.name
}

func (r *PlatformService) Config() *config.ServiceConfig {
	return r.serviceConfig
}

func (r *PlatformService) Context() *Context {
	return r.context
}

func (r *PlatformService) ID() string {
...
func NewService(name string, config *config.ServiceConfig, context *Context) *PlatformService {
...
func (r *PlatformService) PlatformService() (*client.Service, error) {
	return r.FindExisting(r.name)
}

func (r *PlatformService) Create(ctx context.Context, options options.Create) error {
	service, err := r.FindExisting(r.name)
...
func (r *PlatformService) Up(ctx context.Context, options options.Up) error {
	return r.up(true)
}

func (r *PlatformService) Build(ctx context.Context, buildOptions options.Build) error {
	return nil
}

func (r *PlatformService) up(create bool) error {
	service, err := r.FindExisting(r.name)
	if err != nil {
		return err
	}
...
func (r *PlatformService) resolveServiceAndStackId(name string) (string, string, error) {
...
func (r *PlatformService) FindExisting(name string) (*client.Service, error) {
...
func (r *PlatformService) Metadata() map[string]interface{} {
	return rUtils.NestedMapsToMapInterface(r.serviceConfig.Metadata)
}

func (r *PlatformService) HealthCheck(service string) *client.InstanceHealthCheck {
	if service == "" {
		service = r.name
	}
...
func (r *PlatformService) getConfiguredScale() int {
...
func (r *PlatformService) createService() (*client.Service, error) {
...
func (r *PlatformService) setupLinks(service *client.Service, update bool) error {
	// Don't modify links for selector based linking, don't want to conflict
	// Do not modify load-balancer links; the compatibility control plane creates them.
...
func (r *PlatformService) SelectorContainer() string {
	return r.serviceConfig.Labels["io.rancher.service.selector.container"]
}

func (r *PlatformService) SelectorLink() string {
	return r.serviceConfig.Labels["io.rancher.service.selector.link"]
}

func (r *PlatformService) getServiceLinks() ([]client.ServiceLink, error) {
...
func (r *PlatformService) getLinks() (map[Link]string, error) {
...
func (r *PlatformService) containers() ([]client.Container, error) {
	service, err := r.FindExisting(r.name)
	if err != nil {
		return nil, err
	}
...
func (r *PlatformService) Log(ctx context.Context, follow bool) error {
	service, err := r.FindExisting(r.name)
	if err != nil || service == nil {
		return err
	}
...
func (r *PlatformService) pipeLogs(container *client.Container, conn *websocket.Conn) {
	defer conn.Close()
...
func (r *PlatformService) DependentServices() []project.ServiceRelationship {
...
func (r *PlatformService) Client() *client.RancherClient {
	return r.context.Client
}

func (r *PlatformService) pullImage(image string, labels map[string]string) error {
...
func (r *PlatformService) Pull(ctx context.Context) (err error) {
	config := r.Config()
	if config.Image == "" || FindServiceType(r) != PlatformType {
		return
	}
...
func appendHash(service *PlatformService, existingLabels map[string]interface{}) (map[string]interface{}, error) {
	...
	ret[composeConfigHashLabel] = hashValue
	return ret, nil
}
