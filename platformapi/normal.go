...
func (f *NormalFactory) Hash(service *PlatformService) (digest.ServiceHash, error) {
	hash, _, err := f.configAndHash(service)
	return hash, err
}

func (f *NormalFactory) configAndHash(r *PlatformService) (digest.ServiceHash, *CompositeService, error) {
	platformService, launchConfig, secondaryLaunchConfigs, err := f.config(r)
	if err != nil {
		return digest.ServiceHash{}, nil, err
	}

	hash, err := digest.CreateServiceHash(platformService, launchConfig, secondaryLaunchConfigs)
	if err != nil {
		return digest.ServiceHash{}, nil, err
	}

	platformService.LaunchConfig = launchConfig
	platformService.LaunchConfig.Labels[digest.ServiceHashKey] = hash.LaunchConfig
	platformService.Metadata[digest.ServiceHashKey] = hash.Service
	...
	for _, secondaryLaunchConfig := range secondaryLaunchConfigs {
		...
		platformService.SecondaryLaunchConfigs = append(platformService.SecondaryLaunchConfigs, secondaryLaunchConfig)
	}

	return hash, platformService, nil
}

func (f *NormalFactory) config(r *PlatformService) (*CompositeService, *client.LaunchConfig, []client.SecondaryLaunchConfig, error) {
...
func (f *NormalFactory) Create(r *PlatformService) error {
...
func (f *NormalFactory) Rollback(r *PlatformService) error {
...
func (f *NormalFactory) Upgrade(r *PlatformService, force bool, selected []string) error {
	existingService, err := r.FindExisting(r.Name())
	if err != nil || existingService == nil {
		return err
	}
...
func (f *NormalFactory) upgrade(r *PlatformService, existingService *client.Service, service, launchConfig bool, secondaryNames, removedSecondaryNames []string) error {
